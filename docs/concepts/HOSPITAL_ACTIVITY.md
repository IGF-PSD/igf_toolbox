...
- The *effet structure* can also be broken down into four sub-effects: $\boxed{effet \ \ structure_{n} = effet \ \racine_{n} + effet \ \ bascule \\ vers \\ l'ambulatoire_{n} + effet \ \sévérité_{n} + effet \ \résiduel_{n}}$
