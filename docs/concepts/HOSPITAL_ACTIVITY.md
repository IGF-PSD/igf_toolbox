...
- The *effet nombre de séjours* can be broken down into two sub-effects: $effet \ \ nombre \ \ de \ \ séjours_{n} = effet \ \ démographie_{n} + effet \ \ modification \ \ du \ \ recours \ \ à \ \ l'hospitalisation_{n}$ where *effet démographie* corresponds to the change in the number of stays induced by variations in the population of each age group, assuming a constant hospitalization rate per age group. The *effet modification du recours à l'hospitalisation* measures the change in *volume économique* induced by a variation in hospitalization rates per age group, given a fixed population. The *effet démographie* can also be broken down into two components:
  - dfdfdfdfdf
